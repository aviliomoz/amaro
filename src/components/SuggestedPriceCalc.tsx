...
export const SuggestedPriceCalc = () => {
    ...
    const priceBeforeTax = (item.cost_price || 0) / ((item.cost_percentage || 0) / 100)
    const contributionMargin = priceBeforeTax - (item.cost_price || 0)
    const contributionMarginPercentage = (contributionMargin / priceBeforeTax) * 100
    const taxes = priceBeforeTax * (restaurant?.sales_tax! / 100) || 0
    const priceAfterTax = priceBeforeTax + taxes
    const finalPrice = Math.round(priceAfterTax)
    ...
    return <div className="flex flex-col gap-2 text-sm">
        ...
        {show && <>
            ...
            <div className="flex items-center justify-end gap-2">
                <p className="flex items-center gap-2 font-semibold">{`Margen de contribución (${(contributionMarginPercentage / 100).toLocaleString("es-PE", { style: "percent" })}):`}</p>
                <p className="w-28 text-right font-bold">{contributionMargin.toLocaleString("es-PE", { style: "currency", currency: "PEN" })}</p>
            </div>
            <div className="flex items-center justify-end gap-2">
