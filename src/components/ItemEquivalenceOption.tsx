...
import { ItemIngredientDto, ItemUm, APIResponse } from "@amaro-software/core"
...
export const ItemEquivalenceOption = () => {
    ...
    const open = () => {
        setItem({ ...item, equivalence_amount: item.equivalence_amount || 1, equivalence_um: item.equivalence_um || "kilogram" })
    }
    ...
    const deleteEquivalence = async () => {
        try {
            ...
            const { data: uses } = await axiosAPI.get<APIResponse<ItemIngredientDto[]>>(`/items/uses/${item.id}`)
            ...
            setItem({ ...item, equivalence_amount: null, equivalence_um: null })
        } catch (error) {
    ...
    if (!item.equivalence_um || !item.equivalence_amount) return <ShowButton onClick={open}>Agregar equivalencia</ShowButton>
    ...
    return <div className="border rounded-md p-4 text-sm flex items-center gap-4 w-full">
        ...
        <select className="border rounded-md py-1.5 px-3 outline-none cursor-pointer disabled:cursor-not-allowed w-full" id="um" name="um" value={item.equivalence_um || "kilogram"} onChange={(e) => setItem({ ...item, equivalence_um: e.target.value as ItemUm })}>
